...
uniform vec4 u_time;
SAMPLER2D(s_arm_diff, 0);
SAMPLER2D(s_arm_spec, 2);
...
vec3 CalcDirLight(vec3 normal, vec3 viewDir, vec2 v_texcoord0)
{
    ...
    // 合并结果
    vec3 ambient  = dirLight_ambient.xyz  * vec3(texture2D(s_arm_diff, v_texcoord0));
    vec3 diffuse  = dirLight_diffuse.xyz  * diff * vec3(texture2D(s_arm_diff, v_texcoord0));
    vec3 specular = dirLight_specular.xyz * spec * vec3(texture2D(s_arm_spec, v_texcoord0));
...
vec3 CalcSpotLight(vec3 normal, vec3 fragPos, vec3 viewDir, vec2 v_texcoord0)
{
    ...
    // combine results
    vec3 ambient = spotLight_ambient.xyz * vec3(texture2D(s_arm_diff, v_texcoord0));
    vec3 diffuse = spotLight_diffuse.xyz * diff * vec3(texture2D(s_arm_diff, v_texcoord0));
    vec3 specular = spotLight_specular.xyz * spec * vec3(texture2D(s_arm_spec, v_texcoord0));
