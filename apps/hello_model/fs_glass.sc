...
vec3 CalcDirLight(vec3 normal, vec3 viewDir, vec2 v_texcoord0)
{
    ...
    // 合并结果
    vec3 ambient  = dirLight_ambient.xyz  * vec3(texture2D(s_glass_diff, v_texcoord0));
    vec3 diffuse  = dirLight_diffuse.xyz  * diff * vec3(texture2D(s_glass_diff, v_texcoord0));
    vec3 specular = dirLight_specular.xyz * spec * vec3(0.0);
